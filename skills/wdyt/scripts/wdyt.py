...
WDYT_MARKER = "wdyt-review-request"
MARKER_LINE = (
    f"<!-- {WDYT_MARKER}: generated by the wdyt tool, not typed by a human -->\n"
)

# Payloads written by older versions, which predate the marker.
LEGACY_SIGNATURES = (
...
def find_transcript(explicit: str | None, cwd: Path, session_id: str | None = None) -> Path:
    ...
    for candidate in files:
        if not is_wdyt_child(candidate):
            return candidate
    return files[0]


def is_wdyt_child(path: Path) -> bool:
    ...
    try:
        with path.open("r", encoding="utf-8", errors="replace") as fh:
            for _ in range(40):
                ...
                if not line:
                    break
                if WDYT_MARKER in line or any(sig in line for sig in LEGACY_SIGNATURES):
                    return True
    except OSError:
        pass
    return False
...
def parse_transcript(path: Path, keep_thinking: bool) -> list[dict]:
    ...
    with path.open("r", encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            # A previous wdyt run whose payload was logged into this session.
            # Treating it as a human instruction would make each review the
            # input to the next one.
            if WDYT_MARKER in text or any(sig in text for sig in LEGACY_SIGNATURES):
                continue
...
def extract_goal(turns: list[dict], goal_turns: int) -> str:
    """Every instruction the human gave, and nothing the agent said back.
    ...
    """
    humans = [t for t in turns
              if t["role"] == "user" and t["text"].strip().lower() not in BOILERPLATE]
    if goal_turns > 0:
        humans = humans[:goal_turns]
    if not humans:
        return "(no human instructions found in this session)"
    return "\n\n".join(
        f"--- instruction {i} ---\n{clip(t['text'], GOAL_TURN_CAP)}"
        for i, t in enumerate(humans, 1)
    )
...
def ask_cli(backend: str, spec: dict, model: str | None, system: str, user: str,
    ...
    started = time.time()
    try:
        with tempfile.TemporaryDirectory(prefix="wdyt-") as scratch:
            proc = subprocess.run(
                cmd,
                input=f"{system}\n\n{user}",
                capture_output=True,
...
def main() -> None:
    ...
    if args.mode == "fresh":
        ...
        human_turns = sum(1 for t in turns if t["role"] == "user")
        shown = min(human_turns, args.goal_turns) if args.goal_turns else human_turns
